...
| `docs/platforms/gcp-merod.md` | GCP node-image-gcp deployment/verification runbook | `packer/gcp/merod/*`, `release-node-image-gcp.yaml`, `scripts/verify-node-image-gcp-release-assets.sh` | Operators |
...
| `docs/node-image-gcp-policy-promotion-pr.md` | node-image-gcp policy promotion flow | `node-image-gcp-policy-promotion-pr.yaml` | Release engineers |
