...
- **mero-kms can run without dstack, as a TDX cluster** (`MERO_KMS_BACKEND=tdx`; [design](docs/design/gcp-tdx-kms.md)). Keys derive by HKDF from a random root that exists only in the memory of the cluster's replicas, on the same `{namespace}/{profile}/{peerId}` paths, and so does the transport key for sealed release. Quotes come from configfs-tsm. One replica generates the root (`MERO_KMS_BOOTSTRAP`); the others join (`MERO_KMS_PEERS`) through `POST /cluster/nonce` and `POST /cluster/join`. A replica gives the root only to a TD whose quote carries exactly its own MRTD and RTMR0–3, is not a debug TD, and has an allowed TCB status. Both sides attest, and the root travels sealed between one-time X25519 keys. A `tdx` replica takes its node allowlist only from its image (`USE_ENV_POLICY=true`), refuses to start in a debug TD or with RTMR3 never extended, and reports `clusterRootReady` on `/health`. The dstack backend stays the default until the GCP image ships.

### Security

- **Debug TDs are refused.** A TD launched with `TDATTRIBUTES.DEBUG` reports the same measurements as the TD it came from, but its host can read its memory. The cluster join refuses one explicitly; node quotes are refused through core's `calimero-tee-attestation` once mero-kms moves to a core revision that has the check.
