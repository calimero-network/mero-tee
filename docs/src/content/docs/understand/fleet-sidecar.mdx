...
| `FLEET_TOKEN` | **Issued by MDMA on attested registration**, stored at `/mnt/data/fleet/fleet-token` | Never baked into the image. Sent as `X-Fleet-Token` once held; the two bootstrap routes take none. See [How the node gets its fleet token](#how-the-node-gets-its-fleet-token). |
...
There is no shell on a confidential VM, so `/run/calimero/fleet-sidecar.log` is
...
The token is stored at `/mnt/data/fleet/fleet-token` — on the **encrypted data
...
- **The version is durable.** It lives in `/mnt/data/fleet/fleet-recovery.json`,
...
| Reported authorship state | `/mnt/data/fleet/fleet-authorship.json` |
