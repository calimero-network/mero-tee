...
<Aside type="caution">
Changing `fleet_delegated_execution` changes `/etc/traefik/routing.yml` and the
...
| `GET`/`POST /admin-api/contexts/<ctx>/intents` | relays only (`fleet_delegated_execution`) | The body holds a warrant signed by the author's device key, committing to this context, method and arguments. |

Admission is **not** gated on `fleet_delegated_execution`, and that asymmetry is
