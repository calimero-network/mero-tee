...
`fleet-admitted.json` is newer than `fleet-confirmed.json`, and the join loop
...
`/var/lib/calimero/`, so a node upgrading from a build that predates the
...
keys and its `ReadOnlyTee` membership: the very state the admitted set exists to
...
<SeqDiagram
  ...
  steps={[
    ...
    { from: 0, to: 2, label: 'meroctl namespace leave <ID> (admitted − desired)', dashed: true },
    { from: 2, to: 2, label: 'MemberLeft → evict + purge keys' },
    { type: 'divider', label: 'confirmed/admitted ∩= desired; persist, sleep 1s, repeat' },
  ]}
/>
...
The broadcast only admits the node if a peer allowed to vouch — an admin, or a
TEE already admitted — is in the namespace's gossip mesh to hear it. When the
only such peer is an owner's laptop behind a relay, that mesh may form late or
never, and the retry above just keeps announcing into it.
...
On-disk bytes ride it too, as `bytes: {state, private_state, delta, governance,
total}`, read once per scan from merod's `GET /admin-api/usage` over loopback
