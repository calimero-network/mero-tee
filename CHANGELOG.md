...
- **Every node profile verifies its KMS against its own profile's policy** (calimero-network/core#4069). `calimero-init` set `MERO_TEE_PROFILE` only on `locked-read-only`, because merod fetched the release's default policy asset, which is the `locked-read-only` KMS policy for every node profile: a `debug` or `debug-read-only` node could only be checked against the locked KMS's measurements, never against its own profile's KMS. merod now fetches `kms-phala-attestation-policy.<profile>.json` for `MERO_TEE_PROFILE` (the per-profile assets this release already signs and publishes), so `calimero-init` exports `MERO_TEE_PROFILE` as the image's profile on every profile, and merod refuses a policy for any other profile. `scripts/ci/tests/calimero-init-disk-encryption-test.sh` now asserts the pin on `debug` and `debug-read-only`.

  **Requires a core release with per-profile policy selection** (calimero-network/core#4069) and a `merodVersion` bump. The baked `0.11.0-rc.47` fetches only the default asset, so on it a debug node with a KMS would refuse the locked policy and fail to boot; `locked-read-only` is unaffected either way.
