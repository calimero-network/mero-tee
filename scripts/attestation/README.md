...
| **`kms/`** | Phala/dstack-specific checks: compose hash from event log (`verify_dstack_compose_hash.py`), local compose-hash flow helper (`verify-compose-hash-flow.sh`), Phala probe artifact inspection (`fetch-and-inspect-phala-probe-event-log.sh`). |
