...
expected_node_release_tag="$(jq -r '.node_image_tag // .version // empty' <<< "${release_entry_json}")"
...
if [[ -z "${expected_node_release_tag}" ]]; then
...
jq -n \
  ...
  --arg node_image_tag "${expected_node_release_tag}" \
